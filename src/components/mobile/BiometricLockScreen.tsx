...
export function BiometricLockScreen({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0a0f1d] text-white flex flex-col items-center justify-between py-10 px-6 animate-fade-in overflow-y-auto">
      {/* Top App Brand */}
      <div className="flex flex-col items-center gap-2 pt-4">
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center text-white shadow-xl shadow-emerald-950/40">
      ...
      <div className="w-full max-w-xs flex flex-col items-center space-y-4 my-auto">
        {/* PIN Dots */}
      ...
      {/* Numeric Keypad */}
      <div className="w-full max-w-xs grid grid-cols-3 gap-3 pb-2">
        {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(num => (
          <button
            key={num}
            type="button"
            onClick={() => handleKeyPress(num)}
            className="w-14 h-14 rounded-full bg-gray-800/60 hover:bg-gray-700/80 text-xl font-bold text-white flex items-center justify-center mx-auto transition-transform active:scale-95 shadow-md border border-gray-700/40"
          >
            {num}
          </button>
        ))}
        ...
        <button
          ...
          className="w-14 h-14 rounded-full bg-transparent hover:bg-white/10 text-gray-400 flex items-center justify-center mx-auto transition-transform active:scale-95"
        >
